...
/**
 * [TODO: check documentation and make necessary changes made during
 * refactoring]
 * 
 * A basic client, using {@link SocketConnection} and handler delegate
 * <p>
 * This class is simply an assembly of various other components that address
 * distinct concerns of a JRedis connection, and effectively defines the
 * connection policy and use-case patterns by selecting this set of cooperating
 * elements.
 * <p>
 ...
 * synchronised/sequential access to the api defined by {@link JRedis}. Both the
 * connection and protocol handler delegates of this class are intended for use
 * by a <b>single</b> thread, or strictly sequential access by a pool of
 * threads. You can create multiple instances of this class and use dedicated
 * threads for each to create a service (if you wish).
 * <p>
 * Redis protocol is handled by a {@link Protocol} instance obtained from the
 * {@link ProtocolManager}. This class will by default specify the
 * {@link RedisVersion#current_revision}.
 * 
...
public class JRedisClient extends SynchJRedisBase {
    ...
    public JRedisClient(ConnectionSpec connectionSpec) {
        ...
        Connection synchConnection = createSynchConnection(connectionSpec,
                false, RedisVersion.current_revision);
        setConnection(synchConnection);
    }

    /**
     * Connects to the localhost:6379 redis server using the password. Will
     * select db 0.
     * 
     * @param password
     *            used for AUTH
     ...
     * obtained from the {@link ProtocolManager} and using localhost:6379 as its
     * network addressing parameters. Database will be selected to db 0 Assumes
     ...
     *            redis server's port
     * @param password
     *            to use for AUTHentication (can be null)
     * @param database
    ...
    public JRedisClient(String host, int port, String password, int database)
            throws ClientRuntimeException {
        this(DefaultConnectionSpec.newSpec(host, port, database,
                getCredentialBytes(password)));
    }
    ...
    protected final void setConnection(Connection connection) {
        this.connection = Assert.notNull(connection,
                "connection on setConnection()", ClientRuntimeException.class);
    }
